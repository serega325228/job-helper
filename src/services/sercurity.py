...
class SecurityService:
    def __init__(self, loaded_from: Path):
        self._loaded_from = loaded_from
        self._fernet = self._load_fernet()

    def _secret_path(self) -> Path:
        return self._loaded_from / ".secret_key"

    def _write_secret(self, path: Path, key: bytes, *, exclusive: bool = False) -> None:
    ...
    def _load_fernet(self) -> Fernet:
        ...
        if path.exists():
            ...
            try:
                self._write_secret(path, key, exclusive=True)
